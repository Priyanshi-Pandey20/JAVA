public class CircularLL {
    ...
    static Node addToEmpty(Node tail, int data) { // add node to empty linked list pblm
        if (tail != null)
            return tail;
    ...
    static Node addEnd(Node tail, int data) { // add node at last in the linked list pblm 
        if (tail == null) {
            return addToEmpty(tail, data);
        }
        Node newNode = new Node();
    ...
    static Node deleteNode(Node tail,int key){  // delete  node from linked list pblm
        if( tail== null){
            return null;
        }
    ...
    static void traverse(Node tail){ // traverse the node in linkedlist  pblm 
        Node p;
        if(tail == null){
