...
class Recursion {
    ...
    public static boolean isSorted(int arr[], int i) { ///check if array sorted or not pblm  
        if (i == arr.length - 1)
            return true;
        if (arr[i] > arr[i + 1]) {
            return false;
    ...
    public static void TowerOfHanoi(int n, char A, char B, char C) { // tower  of hanaoi pblm
        if (n == 0) {
            return;
        }
